...
use App\Models\WorkshopModel as WorkshopModelEntity;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\Rule;

class UpdateWorkshopModelRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $model = $this->route('workshop_model');
        $modelId = $model instanceof WorkshopModelEntity ? $model->getKey() : $model;
        ...
        return [
            ...
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('workshop_models', 'name')
                    ->ignore($modelId)
                    ->where(function ($query) use ($companyId, $brandId): void {
                        if ($companyId) {
                            $query->where('company_id', $companyId);
                        }
                        if ($brandId) {
