<x-layouts.dashboard-layout :meta="$meta ?? []">
    <div class="w-full p-4 pb-20 md:p-6 md:pb-6">
        <!-- Breadcrumb Start -->
        <div x-data="{ pageName: `{{ gettext('Nota de Venta') }}` }">
            <div class="flex flex-wrap items-center justify-between gap-3 pb-6">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90" x-text="pageName">{{ gettext('Nota de Venta') }}</h2>
                <nav>
                    <ol class="flex items-center gap-1.5">
                        ...
                        <li>
                            <a class="inline-flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400" href="{{ route('sales.sales_notes.index') }}">
                                {{ gettext('Notas de Venta') }}
                                <svg class="stroke-current" width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M6.0765 12.667L10.2432 8.50033L6.0765 4.33366" stroke="" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"></path>
                                </svg>
                            </a>
                        </li>
                        <li class="text-sm text-gray-800 dark:text-white/90" x-text="pageName">{{ gettext('Nota de Venta') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="w-full rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="p-5 xl:p-8">
                    <div class="mb-9 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <h5 class="mb-2 text-base font-semibold text-gray-800 dark:text-white/90">
                                {{ $invoice->company->name ?? $invoice->company->legal_name }}
                            </h5>
                            <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
                                @if($invoice->company->address)
                                    {{ $invoice->company->address }}
                                @endif
                                @if($invoice->branch && $invoice->branch->address)
                                    <br>{{ $invoice->branch->address }}
                                @endif
                            </p>
                            ...
                            <span class="block text-sm text-gray-500 dark:text-gray-400">
                                {{ $invoice->issue_date->translatedFormat('d F, Y') }}
                            </span>
                        </div>
                        ...
                        <div class="sm:text-right">
                            ...
                            <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
                                ...
                                @if($invoice->customer->document_number)
                                    <br>{{ gettext('Documento') }}: {{ $invoice->customer->document_number }}
                                @endif
                            </p>
                            <span class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                {{ gettext('Vence el') }}:
                            ...
                            <span class="block text-sm text-gray-500 dark:text-gray-400">
                                {{ $invoice->due_date ? $invoice->due_date->translatedFormat('d F, Y') : '-' }}
                            </span>
                        </div>
                    </div>
                        ...
                        <div class="w-[220px]">
                            ...
                            <ul class="space-y-2">
                                <li class="flex justify-between gap-5">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-400">USD {{ number_format($invoice->subtotal, 2, '.', ',') }}</span>
                                </li>
                                <li class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ gettext('IVA (15%)') }}:
                                    </span>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-400">USD {{ number_format($invoice->tax_amount, 2, '.', ',') }}</span>
                                </li>
                                <li class="flex items-center justify-between">
                                    <span class="font-medium text-gray-700 dark:text-gray-400">
                                        {{ gettext('Total') }}
                                    </span>
                                    <span class="text-lg font-semibold text-gray-800 dark:text-white/90">USD {{ number_format($invoice->total_amount, 2, '.', ',') }}</span>
                                </li>
                            </ul>
                        </div>
                    </div>
                    ...
                    @if($invoice->payments->isNotEmpty())
                        <div class="mb-6">
                            <h4 class="mb-3 text-sm font-semibold text-gray-900 dark:text-white">{{ gettext('Pagos') }}</h4>
                            <div class="overflow-x-auto rounded-xl border border-gray-100 dark:border-gray-800">
                                <table class="min-w-full text-left text-gray-700 dark:text-gray-400">
                                    <thead class="bg-gray-50 dark:bg-gray-900">
                                        <tr class="border-b border-gray-100 dark:border-gray-800">
                                            ...
                                            <th class="px-5 py-3 text-left text-sm font-medium whitespace-nowrap text-gray-700 dark:text-gray-400">
                                                {{ gettext('Fecha') }}
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                        @foreach($invoice->payments as $payment)
                                            <tr>
                                                ...
                                                <td class="px-5 py-3 text-right text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <a
                            href="{{ route('sales.sales_notes.index') }}"
