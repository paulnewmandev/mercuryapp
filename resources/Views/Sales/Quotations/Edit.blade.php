<x-layouts.dashboard-layout :meta="$meta ?? []">
    <x-ui.breadcrumb :title="gettext('Editar Cotización')" :items="$breadcrumbItems" />

    <section class="w-full space-y-6 pb-12" data-quotation-form-root data-quotation-id="{{ $quotation->id }}">
        <form method="POST" action="{{ route('sales.quotations.update', $quotation->id) }}" class="space-y-6" novalidate>
            @csrf
            @method('PATCH')
                        ...
                        <div class="relative" data-quotation-customer>
                            <div class="flex w-full items-center gap-3 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 transition focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-gray-700 dark:bg-slate-900 dark:text-white">
                                ...
                                <input
                                    ...
                                    value="{{ $quotation->customer->display_name }}"
                                    required
                                >
                                <input type="hidden" name="customer_id" data-quotation-customer-id value="{{ $quotation->customer_id }}" required>
                            </div>
                            <div
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Wait for initQuotationForm to complete
                setTimeout(() => {
                    const root = document.querySelector('[data-quotation-form-root]');
                    if (!root) return;

                    // Access the cartItems array from the module scope
                    // We'll use a custom event to initialize the cart
                    @if($quotation->items->isNotEmpty())
                        @php
                            ...
                            $services = \App\Models\Service::whereIn('id', $itemIds)->get()->keyBy('id');
                            
                            $existingItemsData = $quotation->items->map(function($item) use ($products, $services) {
                                $name = '';
                                $sku = '';
                        ...
                        // Dispatch event to load existing items
                        window.dispatchEvent(new CustomEvent('quotation:loadItems', { detail: existingItems }));
                    @endif
                }, 500);
            });
        </script>
