<x-layouts.dashboard-layout :meta="$meta ?? []">
    <x-ui.breadcrumb
        :title="gettext('Editar movimiento')"
    ...
    >
        <article class="rounded-3xl border border-gray-200 bg-white/90 p-6 shadow-sm backdrop-blur dark:border-gray-800 dark:bg-slate-900/80">
            <div class="flex items-center justify-between mb-6">
                <a
                    href="{{ route('inventory.product_transfers.show', $transfer) }}"
            ...
            <form
                method="POST"
                action="{{ route('inventory.product_transfers.update', $transfer) }}"
                data-product-transfer-form
                data-entry-id="{{ $transfer->id }}"
                class="space-y-6"
                novalidate
            >
                @csrf
                @method('PATCH')
                <input type="hidden" name="id" data-product-transfer-input="id" value="{{ $transfer->id }}">
                <input type="hidden" name="status" data-product-transfer-input="status" value="{{ $transfer->status }}">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('movement_date', $transferData['movement_date'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            value="{{ old('reference', $transferData['reference'] ?? '') }}"
                            class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-gray-700 dark:bg-slate-900 dark:text-white"
                            maxlength="100"
                        >
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        >
                            <input type="hidden" name="origin_warehouse_id" data-product-transfer-input="origin_warehouse_id" value="{{ old('origin_warehouse_id', $transferData['origin_warehouse_id'] ?? '') }}">
                            <button
                                type="button"
                    ...
                    <div>
                        ...
                        >
                            <input type="hidden" name="destination_warehouse_id" data-product-transfer-input="destination_warehouse_id" value="{{ old('destination_warehouse_id', $transferData['destination_warehouse_id'] ?? '') }}">
                            <button
                                type="button"
                                class="flex w-full items-center justify-between rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-800 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-gray-700 dark:bg-slate-900 dark:text-white"
                                data-select-trigger
                ...
                <div>
                    ...
                    >{{ old('notes', $transferData['notes'] ?? '') }}</textarea>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-800">
                    ...
                    >
                        <span data-product-transfer-submit-label>{{ gettext('Actualizar') }}</span>
                    </button>
                </div>
            </form>
        </article>
