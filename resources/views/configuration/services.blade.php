<x-layouts.dashboard-layout :meta="$meta ?? []">
    <x-ui.breadcrumb :title="gettext('Servicios')" :items="$breadcrumbItems" />
    ...
    <section
        ...
        data-services-root
        data-services-store-url="{{ route('configuration.services.store') }}"
        data-services-base-url="{{ url('configuration/services') }}"
        data-services-table-id="services-table"
        data-services-categories='@json($categories->map(fn ($category) => ['id' => $category->id, 'name' => $category->name]))'
        data-services-category-options-url="{{ route('configuration.service_categories.options') }}"
    >
        <x-ui.data-table
            tableId="services-table"
            apiUrl="{{ route('configuration.services.data') }}"
            :columns="$columns"
            ...
            <div class="mx-4 my-8 w-full max-w-3xl">
                <div class="relative w-full overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-slate-900">
                    <div class="relative border-b border-gray-200 px-6 py-4 text-center dark:border-gray-800">
                        <h3 class="text-base font-medium text-gray-900 dark:text-white" data-service-modal-title></h3>
                        <button
                            type="button"
                    ...
                    <form data-service-form class="space-y-6 px-6 py-6" novalidate>
                        <input type="hidden" data-service-input="id">
                        <input type="hidden" data-service-input="status" value="A">
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div class="md:col-span-2">
                                ...
                                >
                                    <input type="hidden" data-service-input="category_id" value="">
                                    <button
                                        type="button"
                                    ...
                                    <div
                                        ...
                                        hidden
                                    >
                                        <div class="max-h-60 overflow-y-auto py-2" data-service-category-options>
                                            @foreach ($categories as $category)
                                                <button
                                                    type="button"
                                                    class="group flex w-full items-center justify-between px-4 py-2 text-left text-sm text-heading transition hover:bg-primary-soft/20 data-[selected=true]:bg-primary-soft/30 data-[selected=true]:text-primary dark:text-white"
                                                    data-select-option
                                ...
                                <p data-service-error="category_id" class="mt-1 text-xs text-rose-500"></p>
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <input
                                    ...
                                    required
                                >
                                <p data-service-error="name" class="mt-1 text-xs text-rose-500"></p>
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <textarea
                                    data-service-input="description"
                                ...
                                ></textarea>
                                <p data-service-error="description" class="mt-1 text-xs text-rose-500"></p>
                            </div>
                            <div>
                                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-300">{{ gettext('Precio') }}</label>
                                <input
                                    type="number"
                                    data-service-input="price"
                                    step="0.01"
                                    min="0"
                                    class="w-full rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-gray-700 dark:bg-slate-900 dark:text-white"
                                    placeholder="0.00"
                                >
                                <p data-service-error="price" class="mt-1 text-xs text-rose-500"></p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-800">
                            ...
                            <button
                                ...
                                data-service-submit
                            >
                                <span data-service-submit-label>{{ gettext('Guardar') }}</span>
                            </button>
                        </div>
                    </form>
                </div>
        ...
        >
            <div class="mx-4 my-8 w-full max-w-3xl">
                <div class="relative w-full overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-slate-900">
                    ...
                    <div class="space-y-4 px-6 py-6" data-service-view>
                            ...
                            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-slate-900/60">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white" data-service-view-field="category_name"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-slate-900/60">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white" data-service-view-field="name"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-slate-900/60 md:col-span-2">
                                <div class="flex items-start gap-3">
                                    <span class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-soft text-primary">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white" data-service-view-field="description"></p>
                                    </div>
                                </div>
                            </div>
                            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-slate-900/60">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex-1">
                                        <span class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ gettext('Precio') }}</span>
                                        <p class="mt-1 text-sm text-gray-900 dark:text-white" data-service-view-field="price"></p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        >
            <div class="mx-4 my-8 w-full max-w-md">
                <div class="relative w-full overflow-hidden rounded-2xl bg-white shadow-xl dark:bg-slate-900">
                    <div class="relative border-b border-gray-200 px-6 py-4 text-center dark:border-gray-800">
                        <h3 class="text-base font-medium text-gray-900 dark:text-white">{{ gettext('Eliminar servicio') }}</h3>
                        <button
                            type="button"
                            class="absolute right-4 top-1/2 inline-flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-slate-800"
                            data-service-modal-close
                    ...
                    <div class="space-y-4 px-6 py-6">
                        <p class="text-sm text-gray-700 dark:text-gray-300">
                            {{ gettext('¿Estás seguro de eliminar el servicio? Esta acción no se puede deshacer.') }}
                        </p>
                        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                            ...
                            <span>{{ gettext('Se eliminará:') }} <strong data-service-delete-name></strong></span>
                        </div>
                    </div>
